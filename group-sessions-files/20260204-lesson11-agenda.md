## CtD Group Mentor Session Agenda for Lesson 11 / A Front End for the Jobs API

Wednesday, 04 February 2026

### Welcome!

### Housekeeping Items

1. (Briefly) Go over this agenda. It is also found [here](https://github.com/JamieBort/06-jobs-api/blob/week11/group-sessions-files/20260204-lesson11-agenda.md). // TODO: Update the URL to this link.

2. Any Questions - Interrupt me at any time for questions or comments.

3. Explain the [Resources](#resources-were-using-today) below.

4. I will go over the new code for this week (which is found in the [public/](../public/) directory).

   These are the [notes](20260204-lesson11-notes.md) associated with that new code/those files.

### Resources we're using today

- (_No video this week_)
- [MongoDB](https://www.mongodb.com/) (_No new database material this week_)
- [A Front End for the Jobs API](https://classes.codethedream.org/course/node-v3/lion?week=13&lesson=A%20Front%20End%20for%20the%20Jobs%20API) Lesson.
- (_Nothing new with Render.com this week._)
- My [notes](20260204-lesson11-notes.md) for this session.
- The [branch](https://github.com/JamieBort/06-jobs-api/tree/week11) containing my code, agenda, etc. for this lesson.
- Link to today's video: https://youtu.be/-PsdegHbjCE
